...
const playerCards = [];
const players = [];
const colors = ["#fef1c7","#c7e0f2","#d4c7f2","#f2c7c7"];
...
function positionPlayers(){
  const w = playerArea.clientWidth;
  const h = playerArea.clientHeight;
  const positions = {
    2:[{top:"20%", left:"20%"},{top:"20%", left:"calc(100% - 170px)"}],
    3:[{top:"10%",left:"10%"},{top:"10%",left:"calc(100% - 170px)"},{top:"calc(100% - 190px)", left:"calc(50% - 75px)"}],
...
function findPlayerByName(text){
  for(let i=0;i<players.length;i++){
    const name=playerCards[i].querySelector(".name").value.toLowerCase();
...
function parseCommand(text){
  const numMatch=text.match(/\d+/); if(!numMatch) return; const value=parseInt(numMatch[0]);
